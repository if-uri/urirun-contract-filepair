...
//
// Konsument w Go dla file/command/restore. Czyta TEN SAM contracts.json co producent Python,
// egzekwuje tę samą bramę wejścia/wyjścia i odpowiada na ten sam transport — różni się TYLKO
// językiem. Kontrakt jest niezależny od języka; orchestrator widzi tylko JSON zgodny z out.
package main
...
const ROUTE = "file/command/restore"
...
// ── ciało handlera ──────────────────────────────────────────────────────────
func restoreHandler(snap map[string]interface{}) map[string]interface{} {
	path, _ := snap["path"].(string)
	did := "?"
	...
	return map[string]interface{}{
		"ok": true, "connector": "filepair-go", "action": "file-restore",
		"did": fmt.Sprintf("restore(%s)", did), "reversible": true,
		"inverse": map[string]interface{}{
			"path": "file/command/snapshot-delete", "args": args,
		},
	}
}
...
func handleRun(w http.ResponseWriter, r *http.Request) {
	...
	snap, ok := payload["snapshot"].(map[string]interface{})
	if !ok {
		writeJSON(w, 422, map[string]interface{}{"ok": false, "error": fmt.Sprintf("input violates %s: snapshot must be obj", ROUTE)})
		return
	}
